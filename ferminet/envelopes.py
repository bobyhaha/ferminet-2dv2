...
class EnvelopeLabel(enum.Enum):
  ...
  STO = enum.auto()
  STO_POLY = enum.auto()
  DIAGONAL_2D = enum.auto()
  STO_2D  =enum.auto()
  STO_POLY_2D = enum.auto()
...
def _apply_covariance(x: jnp.ndarray, y: jnp.ndarray) -> jnp.ndarray:
  ...
  #only sum over k so that out_{ijmn} = \sum_{k}x_{ijk}y_{kmjn}
...
def make_diagonal_envelope_2d() -> Envelope:
  """Creates a diagonal exponentially-decaying multiplicative envelope."""
...
def make_full_envelope_2d() -> Envelope:
  """Computes a fully anisotropic exponentially-decaying envelope."""
...
def make_sto_envelope_2d() -> Envelope:
  """Creates a Slater-type orbital envelope: exp(-sigma*r_ae) * r_ae^n * pi."""

  def init(
      natom: int, output_dims: int, ndim: int = 2
  ) -> Mapping[str, jnp.ndarray]:
    pi = jnp.zeros(shape=(natom, output_dims))
    sigma = jnp.tile(jnp.eye(ndim)[..., None, None], [1, 1, natom, output_dims])
    # log order of the polynomial (initialize so the order is near zero)
...
def make_sto_poly_envelope_2d() -> Envelope:
  """Creates a Slater-type orbital envelope."""
...
def get_envelope(
  ...
  envelope_builders = {
      ...
      EnvelopeLabel.STO_2D: make_sto_envelope,
      EnvelopeLabel.STO_POLY_2D: make_sto_poly_envelope_2d,
      EnvelopeLabel.DIAGONAL_2D: make_diagonal_envelope_2d,
  }
