...
def init_electrons(  # pylint: disable=dangerous-default-value
    ...
    batch_size: int,
    init_width: float,
    core_electrons: Mapping[str, int] = {}, #dict like obj with key str and value int
    max_iter: int = 10_000,
  ...
  """
  niter = 0
  #core electron is for reducing the electrons in the inner shell e.g. 'Li':2
  ...
  else: # core electron are assume to be paired
    atomic_spin_configs = [
        (atom.element.nalpha - core_electrons.get(atom.symbol, 0) // 2,
         atom.element.nbeta - core_electrons.get(atom.symbol, 0) // 2)
        for atom in molecule
    ] #a list containing the alpha and beta electrons for each molecule
    assert sum(sum(x) for x in atomic_spin_configs) == sum(electrons)
    #tries to correct spin mismatch
    while (
  ...
  if tuple(sum(x) for x in zip(*atomic_spin_configs)) == electrons:
    ...
    for i in range(2): #spin
      for j in range(len(molecule)): #molecule
        atom_position = jnp.asarray(molecule[j].coords) #converts to jax array
        electron_positions.append(
            jnp.tile(atom_position, atomic_spin_configs[j][i])) #for each electron and j has alpha and beta
  ...
  # atom. This part is weird
  key, subkey = jax.random.split(key) #size returns the number of electrons?
  electron_positions += (
      jax.random.normal(subkey, shape=(batch_size, electron_positions.size))
      * init_width
  )
  print("electron pos", electron_positions.shape)
  #(256, 4)
...
def train(cfg: ml_collections.ConfigDict, writer_manager=None):
  ...
  print("batch atom and charges", atoms.shape, charges.shape)
  # (1, 2), (1,)
  ...
  #functions are treated as attributes of modules or classes
  #module = importlib.import_module("my_module") load module by name
  if cfg.network.make_feature_layer_fn:
    ...
    make_feature_layer: networks.MakeFeatureLayer = getattr(
        feature_layer_module, feature_layer_fn
    ) #get the feature layer from the module
  ...
  if cfg.system.get('states', 0): #excited states
  ...
  if ckpt_restore_filename:
    ...
    print("total host", total_host_batch_size)
    ...
    #pos of shape (1, 256, 4) and spin of (1, 256, 2)
  ...
  mcmc_step = mcmc.make_mcmc_step_2d(
      batch_network,
      device_batch_size,
  ...
  with writer_manager as writer:
    ...
    for t in range(t_init, cfg.optim.iterations):
      ...
      data, params, opt_state, loss, aux_data, pmove = step(
          ...
          subkeys,
          mcmc_width)
      # print(data.positions.shape) #(1, 256, 4)
