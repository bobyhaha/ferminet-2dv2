...
def local_kinetic_energy(
  ...
  logabs_f = utils.select_output(f, 1) #select callable function
  ...
  if laplacian_method == 'default':
    ...
    def _lapl_over_f(params, data):
      ...
      eye = jnp.eye(n)
      grad_f = jax.grad(logabs_f, argnums=1) #take grad over the positions. shape: (nelectrons*ndim,)
      #For H4 it's (12, ) so it's (ndim*n_electrons,)
      def grad_f_closure(x):
        return grad_f(params, x, data.spins, data.atoms, data.charges)
      #position of shape (batch, n_walkers, ndim*n_electrons)
      primal, dgrad_f = jax.linearize(grad_f_closure, data.positions)
      #dgrad_f is the Hessian that is diagonal and dgrad_f(v) returns the Hessian vector product 
      #dgrad should have (nelectrons*ndim, nelectrons*ndim)
...
def potential_electron_electron(r_ee: Array) -> jnp.ndarray:
  ...
  Args:
    r_ee: Shape (neletrons, nelectrons, :). r_ee[i,j,0] gives the distance
      ...
      required.
  """
  #jnp.triu_indices_from(r_ee[...,0],1) returns a tuple of position i and j
  #(nelectron, nelectron, something)
  U = 1.0
  sigma = 1/2
  norm = U / (2 * jnp.pi * sigma**2)
  r_ee = r_ee[jnp.triu_indices_from(r_ee[..., 0], 1)] #1 is going up the diag
  return (-norm * jnp.exp(-0.5 * (r_ee)**2/sigma**2)).sum()
...
def potential_electron_nuclear(charges: Array, r_ae: Array) -> jnp.ndarray:
  ...
  """
  #Note that docstring above is misleading where it has one extra dim
  #(nelectron, natoms, something)
  print("Shape of r_ae", r_ae.shape)
  #-jnp.sum(charges / r_ae[..., 0])
  alpha = 1.0
  return -jnp.sum(charges * jnp.exp(-alpha * r_ae))
...
def potential_nuclear_nuclear(charges: Array, atoms: Array) -> jnp.ndarray:
  ...
  """
  #(natom, natom)
  #we can also use jax.expand_dims(atoms, axis=1)
  r_aa = jnp.linalg.norm(atoms[None, ...] - atoms[:, None], axis=-1) #(n_atoms, n_atoms, ndim)
  print("Shape of r_aa", r_aa.shape)
  # jnp.sum(
  #     jnp.triu((charges[None, ...] * charges[..., None]) / r_aa, k=1))
  alpha = 1.0
  return jnp.sum(
      jnp.triu((charges[None, ...] * charges[..., None]) * jnp.exp(-alpha * r_aa), k=1))
