...
def find_last_checkpoint(ckpt_path: Optional[str] = None) -> Optional[str]:
  ...
  if ckpt_path and os.path.exists(ckpt_path):
    ...
    for file in sorted(files, reverse=True):
      ...
      with open(fname, 'rb') as f: #.npz is stored in binary. If .ppz contains Python object then must use allow_pickle
        try:
...
def save(save_path: str,
  ...
  with open(ckpt_filename, 'wb') as f:
    np.savez(
        f,
        t=t,
        data=dataclasses.asdict(data), #mcmc walker config as dataclasses obj
        params=params,
