    ...
    def step_fn(i, x):
      return inner_fun(
          ...
          *x,
          ...
          blocks=blocks,
          i=i)
    # jax.debug.print("______________")
    # jax.debug.print(pos.shape, data.spin.shape, data.atom.shape)
    nsteps = steps * blocks
...
def make_mcmc_step_2d(batch_network,
                   batch_per_device,
                   steps=10,
                   atoms=None,
                   ndim=2,
  ...
  def mcmc_step(params, data, key, width):
    ...
    # jdbg.print("pos {} spins {} atoms {} charges {}",
    #           jnp.shape(pos),
    #           jnp.shape(data.spins),
    #           jnp.shape(data.atoms),
    #           jnp.shape(data.charges),
    #           )
