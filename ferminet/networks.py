...
def construct_input_features( #this is fine because ndim set to options.ndim
    pos: jnp.ndarray,
    atoms: jnp.ndarray, #I changed to 2 but it actually works? change back if need
...
def make_ferminet_features( #takes in ndim from make_fermi_net
    natoms: int,
...
def construct_symmetric_features(
    ...
    Args:
      h_one: one‐electron features, shape (nelectrons, n1).
      h_two: two‐electron features, shape (nelectrons, nelectrons, n2).
      nspins: tuple (#spin‐up, #spin‐down).
      h_aux: optional auxiliary features, shape (nelectrons, naux).

    Returns:
      Permutation‐equivariant features of shape
      (nelectrons, 3*n1 + 2*n2 + naux) if both spin channels occupied,
      otherwise (nelectrons, 2*n1 + n2 + naux).
    """
...
def make_fermi_net(
  ...
  def apply(
    ...
    """
    print("____________")
    #init with the normal shapes
    print(pos.shape, spins.shape, atoms.shape, charges.shape)
...
# Only override when DEBUG_CSF=1
if os.environ.get("DEBUG_CSF") == "1":
    _orig_csf = construct_symmetric_features  # noqa: F821

    @functools.wraps(_orig_csf)
    def construct_symmetric_features(h_one, h_two, nspins, h_aux=None):
        # Pure‑Python prints to compare TRAIN vs INFERENCE shapes
        print(f"[CSF DEBUG] nspins      = {nspins}")
        print(f"[CSF DEBUG] h_one.shape = {tuple(int(x) for x in h_one.shape)}")
        print(f"[CSF DEBUG] h_two.shape = {tuple(int(x) for x in h_two.shape)}")
        if h_aux is not None:
            print(f"[CSF DEBUG] h_aux.shape = {tuple(int(x) for x in h_aux.shape)}")

        out = _orig_csf(h_one, h_two, nspins, h_aux)

        print(f"[CSF DEBUG] output.shape= {tuple(int(x) for x in out.shape)}")
        print("-" * 40)
        return out

    # Monkey‑patch the module attribute
    import sys
    sys.modules[__name__].construct_symmetric_features = construct_symmetric_features
